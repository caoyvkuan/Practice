...
+ fetch 只有在网络错误或无法链接时才报错
  + 要判断请求是否成功需要通过 Response.status 属性或 Response.ok
...
const response = await fetch(url, {
  ...
  // 默认发送纯文本 标头为 text/plain;charset=UTF-8
  // 如果发送 json 需要添加标头 application/json;charset=utf-8
  // 上传二进制文件 标头会自动修改
...
+ Stream 对象只能读取一次, 也就是 Response 的五个读取方法只能用一个
+ 想用多个就需要使用 clone 进行复制实现多次读取
